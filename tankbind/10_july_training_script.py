...
def get_data(addNoise=None):
    pre = "./"
    new_dataset = TankBindDataSet("/fs/pool/pool-marsot/pdbbind/pdbbind2020/dataset", add_noise_to_com=5)
...
sampler = RandomSampler(train, replacement=True, num_samples=3000)
train_loader = TankBindDataLoader(train, batch_size=5, follow_batch=['x', 'compound_pair'], sampler = sampler)
sampler_2 = RandomSampler(train_after_warm_up, replacement=True, num_samples=3000)
...
for epoch in range(200):
    ...
    for data in data_it:
        ...
        warm_up_epochs = 15
        base_relative_k = 0.01
        if epoch<warm_up_epochs:
            relative_k = base_relative_k * (2**epoch) / (2**warm_up_epochs)
        else:
            relative_k = base_relative_k 
